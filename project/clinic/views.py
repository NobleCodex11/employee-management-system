...
class PatientViewSet(viewsets.ModelViewSet):
    ...
    # Allow all authenticated staff to access patients
    permission_classes = [IsAuthenticated]
...
class AppointmentViewSet(viewsets.ModelViewSet):
    ...
    def pay_bill(self, request, pk=None):
        ...
        bill, created = Bill.objects.get_or_create(appointment=appointment, defaults={'total_amount': 0.00})
        bill.is_paid = True
        bill.save()
        return Response({
...
class LabTestPrescriptionViewSet(viewsets.ModelViewSet):
    ...
    def evaluate(self, request, pk=None):
        ...
        # Check if already evaluated to prevent double-billing
        is_already_evaluated = bool(prescription.lab_test_value)
        ...
        # Generate or update Bill
        bill, created = Bill.objects.get_or_create(appointment=prescription.appointment)
        if not is_already_evaluated:
            bill.total_amount += prescription.lab_test.amount
            bill.save()

        return Response({
            "message": "Lab test evaluated successfully",
            "bill_id": bill.id,
